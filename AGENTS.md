...
- `Models/JevRanker.cs` — pure helpers for the JEV prompt (`BuildState`/`BuildInstructions`) and for ordering candidates by probability; unit tested in `JevRankerTests.cs`. For episodes `BuildState` describes **only the series name** (+ year/season): episode number, episode title and the video file name are deliberately excluded because assrt results are season/series-level packs and the actual episode is chosen after download (`ScoreArchiveEntry`). Movies get title + year + file name.
